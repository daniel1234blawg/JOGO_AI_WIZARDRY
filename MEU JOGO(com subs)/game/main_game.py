...
def main_game(screen, FONT, SMALL_FONT, TINY_FONT, TITLE_FONT, clock):
    ...
    while True:
        ...
        draw_fireball_cooldown(
            screen, player.spells[0], keybinds, 40, 40, 300, 36, SMALL_FONT, MAGIC_GOLD
        )
        draw_hollowpurple_cooldown(
            screen, player.spells[1], keybinds, 40, 90, 450, 50, SMALL_FONT, MAGIC_GOLD
        )
        draw_hud(screen, player, SMALL_FONT)

        if paused:
            draw_pause_menu(screen, FONT, current_width, current_height)
